...
export function parseSolidityCode(code) {
  resetNodeId();
  ...
  try {
    ...
    // Combine all function CFGs with proper positioning
    let yOffset = 0;
    functionGraphs.forEach((funcGraph, index) => {
      ...
      // Add function nodes with offset
      funcGraph.nodes.forEach((node) => {
        nodes.push({
          ...node,
          position: {
            x: node.position.x + xOffset,
            y: node.position.y + yOffset + 80,
          },
        });
      });
...
function buildCFGForFunction(functionNode, functionName) {
  ...
  if (functionNode.body && functionNode.body.statements) {
    const statementResult = processStatements(
...
function processIfStatement(stmt, prevNodeId, exitId, xPosition, yPosition, nodes, edges) {
  ...
  edges.push({
    id: `${conditionNodeId}-${trueBranchId}`,
  ...
  if (stmt.falseBody) {
    ...
    edges.push({
      id: `${conditionNodeId}-${falseBranchId}`,
      source: conditionNodeId,
    ...
    // No else branch, connect directly
    falseEndNodeId = conditionNodeId;
    edges.push({
      id: `${conditionNodeId}-merge-false`,
      source: conditionNodeId,
      target: conditionNodeId,
      type: 'smoothstep',
      label: 'false',
  ...
  nodes.push({
    ...
    position: { x: xPosition, y: maxY },
  });

  edges.push({
    id: `${trueEndNodeId}-${mergeNodeId}`,
    source: trueEndNodeId,
    target: mergeNodeId,
    type: 'smoothstep',
  });
  ...
  if (stmt.falseBody) {
    edges.push({
      id: `${falseEndNodeId}-${mergeNodeId}`,
      source: falseEndNodeId,
      target: mergeNodeId,
      type: 'smoothstep',
    });
  } else {
    edges.push({
      id: `${conditionNodeId}-${mergeNodeId}`,
      source: conditionNodeId,
      target: mergeNodeId,
      type: 'smoothstep',
      label: 'false',
...
function processLoopStatement(stmt, prevNodeId, exitId, xPosition, yPosition, nodes, edges) {
  ...
  nodes.push({
    ...
    position: { x: xPosition - 100, y: bodyY },
  });

  edges.push({
    id: `${conditionNodeId}-${bodyNodeId}`,
    source: conditionNodeId,
    target: bodyNodeId,
    type: 'smoothstep',
    label: 'true',
  ...
  // Loop back edge
  edges.push({
    id: `${bodyEndNodeId}-${conditionNodeId}`,
    source: bodyEndNodeId,
    target: conditionNodeId,
    type: 'smoothstep',
    animated: true,
  });
  ...
  nodes.push({
    ...
    position: { x: xPosition + 100, y: yPosition + 100 },
  });

  edges.push({
    id: `${conditionNodeId}-${exitLoopNodeId}`,
    source: conditionNodeId,
    target: exitLoopNodeId,
    type: 'smoothstep',
    label: 'false',
    style: { stroke: '#f44336' },
  });
